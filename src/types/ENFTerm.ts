 ...
 * e.g., (e_w^2 * w^3) * 5
 */
export class ENFTerm extends OrdinalBase {
    readonly factors: ENFFactor[];
    readonly coefficient: bigint;

    /**
     * @param factors - An array of ENFFactor instances, sorted by descending base.
     * @param coefficient - The finite coefficient of the term.
     */
    constructor(factors: ENFFactor[] = [], coefficient: bigint = 1n) {
        super();
    ...
    simplify(complexityBudget: number = 100, skipMyOwnMPTFCheck = false): { simplifiedOrdinal: OrdinalBase; remainingBudget: number } {
        const myComplexity = this.complexity();
        if (myComplexity <= complexityBudget) {
            return { simplifiedOrdinal: this, remainingBudget: complexityBudget - myComplexity };
        }
        return { simplifiedOrdinal: ZeroOrdinal.instance(), remainingBudget: 0 };
    }
