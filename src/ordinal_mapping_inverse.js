...
function findRemainderHigher(x, k, m, params, threshold) {
    ...
    if (ctx.compare(fr, threshold) < 0) {
        ...
        const rAmplification = ctx.divide(fOmegaK, denominator);
...
export function fInverse(x, params = DEFAULT_F_PARAMS, threshold = 1e-14, depth = 0) {
    const ctx = params.ctx;
...
export function convertFFormatToOrdinalInstance(ord_representation) {
    // Handles:
    // - BigInt n (finite)
    // - "E0_TYPE"
