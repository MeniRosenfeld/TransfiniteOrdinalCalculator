// SimpleParser.js
// Comprehensive parser for ordinal expressions and boolean/comparison operations supporting:
...
// - Successor operator: ordinal' (postfix)
// - Function calls: f[arguments] with functions complexity, toString, parse
// - Variable substitution: expr/.{var:=value, ...}
...
class SimpleParser {
    ...
    _parseSubstitution() {
        let expr = this._parseImplication();
    ...
    _parseComparison() {
        let left = this._parseOrdinalExpression();
    ...
    _parseOrdinalExpression() {
        let left = this._parseOrdinalTerm();

        while (this._peek() && this._peek().type === 'PLUS') {
            this._consume('PLUS');
            const right = this._parseOrdinalTerm();
    ...
    _parseOrdinalTerm() {
        let left = this._parseOrdinalTetration();

        while (this._peek() && this._peek().type === 'STAR') {
            this._consume('STAR');
            const right = this._parseOrdinalTetration();
    ...
    _parseOrdinalTetration() {
        // Handle right-associative tetration
        let base = this._parseOrdinalPower();
        if (this._peek() && this._peek().type === 'DCARET') {
            this._consume('DCARET');
            const height = this._parseOrdinalTetration();
    ...
    _parseOrdinalPower() {
        ...
        if (this._peek() && this._peek().type === 'CARET') {
            this._consume('CARET');
            const exponent = this._parseOrdinalPower(); // right-associative
    ...
    _createBooleanResult(boolValue) {
        return { type: 'boolean', value: Boolean(boolValue) };
    }
    ...
    _valueToString(value) {
        if (this._isOrdinal(value)) {
            ...
            const argsStr = value.args.map(arg => this._valueToString(arg)).join(',');
    ...
    _comparisonOperatorToString(operator) {
        switch (operator) {
            case 'EQ': return '=';
            case 'NEQ': return '!=';
            case 'LT': return '<';
